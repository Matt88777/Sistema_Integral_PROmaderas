...
namespace PROmaderas.AccesoADatos
{
	public class Contexto : DbContext
	{
		...
		protected override void OnModelCreating(ModelBuilder modelBuilder)
		{
			...
			// PLA-HU-018: póliza maestra definida por el script de Sprint 4.
			modelBuilder.Entity<PolizaINSAD>(e =>
			{
				e.ToTable("PolizaINS");
				e.HasKey(x => x.IdPoliza);
				e.Property(x => x.NumeroPoliza).HasMaxLength(50).IsRequired();
				e.Property(x => x.TipoPoliza).HasMaxLength(100).IsRequired();
				e.Property(x => x.Aseguradora).HasMaxLength(100).IsRequired()
					.HasDefaultValue("INS");
				e.Property(x => x.FechaInicio).HasColumnType("date").IsRequired();
				e.Property(x => x.FechaVencimiento).HasColumnType("date").IsRequired();
				e.Property(x => x.MontoAsegurado).HasColumnType("decimal(18,2)");
				e.Property(x => x.Prima).HasColumnType("decimal(18,2)");
				...
				e.HasIndex(x => x.NumeroPoliza).IsUnique();
			});

			// Empleados cubiertos por cada póliza (relación N:M).
			modelBuilder.Entity<EmpleadoPolizaAD>(e =>
			{
				e.ToTable("EmpleadoPoliza");
				e.HasKey(x => x.IdEmpleadoPoliza);
				e.Property(x => x.FechaAsignacion).HasColumnType("date")
					.HasDefaultValueSql("CAST(GETDATE() AS date)");
				e.Property(x => x.FechaExclusion).HasColumnType("date");
				e.Property(x => x.Activa).HasDefaultValue(true);
				e.HasOne(x => x.Empleado).WithMany()
					.HasForeignKey(x => x.IdEmpleado)
					.OnDelete(DeleteBehavior.Restrict);
				e.HasOne(x => x.Poliza).WithMany(x => x.EmpleadosAsignados)
					.HasForeignKey(x => x.IdPoliza)
					.OnDelete(DeleteBehavior.Restrict);
				e.HasIndex(x => new { x.IdEmpleado, x.IdPoliza }).IsUnique();
				e.HasIndex(x => x.IdPoliza);
			});

			// PLA-HU-014: tabla existente; las columnas adicionales se agregarán
			// mediante una migración incremental después de alinear el modelo.
			modelBuilder.Entity<IncapacidadAD>(e =>
			{
				e.ToTable("Incapacidad");
				e.HasKey(x => x.IdIncapacidad);
				e.Property(x => x.TipoIncapacidad).HasMaxLength(100).IsRequired();
				e.Property(x => x.FechaInicio).HasColumnType("date").IsRequired();
				e.Property(x => x.FechaFin).HasColumnType("date").IsRequired();
				e.Property(x => x.NumeroCertificado).HasMaxLength(100).IsRequired();
				e.Property(x => x.EntidadEmisora).HasMaxLength(50).IsRequired();
				e.Property(x => x.Observacion).HasMaxLength(250);
				e.Property(x => x.Activa).HasDefaultValue(true);
				e.Property(x => x.FechaRegistro).HasColumnType("datetime")
					.HasDefaultValueSql("GETDATE()");
				e.HasOne(x => x.Empleado).WithMany()
					.HasForeignKey(x => x.IdEmpleado)
					.OnDelete(DeleteBehavior.Restrict);
