...
public class AdminUsuariosController : Controller
{
    ...
    public async Task<IActionResult> Index()
    {
        ...
        foreach (var usuario in usuarios)
        {
            ...
            var invClaim = claims.FirstOrDefault(c => c.Type == "UltimaInvitacionUtc");
            ...
            resultado.Add(new AdminUserListItemViewModel
            {
                ...
                UltimaInvitacionUtc = invClaim is null
                    ? null
                    : DateTime.Parse(invClaim.Value, null,
    ...
    private async Task CargarEmpleadosActivosAsync(AdminCreateUserViewModel model)
    {
        ...
        model.EmpleadosActivos = empleados
            .Where(e => e.Estado == true && e.IdEmpleado.HasValue)
            .OrderBy(e => e.Nombre)
            .Select(e => new SelectListItem
            {
                Value = e.IdEmpleado!.Value.ToString(),
                Text = $"{e.Nombre ?? string.Empty} - {e.Correo ?? string.Empty}"
            })
            .ToList();
    }
    ...
    private async Task<MailMessage> CrearCorreoInvitacionAsync(UsuarioIdentity usuario)
    {
        ...
        var mail = _mailStore.Add(new MailMessage
        {
        ...
        const string claimType = "UltimaInvitacionUtc";
        var existing = (await _userManager.GetClaimsAsync(usuario))
            .FirstOrDefault(c => c.Type == claimType);
        var nuevo = new Claim(claimType, mail.CreatedAtUtc.ToString("O"));

        if (existing is not null)
            await _userManager.ReplaceClaimAsync(usuario, existing, nuevo);
        else
            await _userManager.AddClaimAsync(usuario, nuevo);
